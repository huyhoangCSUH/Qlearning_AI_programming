...
def exp1(alpha, gamma):	

	
	# Experiment 1: 3000 pRandom then 3000 pGreedy	
	Q = {}
	init_Q(Q)
	PD_world, pickups, dropoffs = init_PD_world()
	current_state = '(1,5)'
	prev_terminated_step = 0
	for i in range(3000):		
		if dropoffs['(4,4)*'] < 8 or dropoffs['(5,1)*'] < 8:
			...
			print "Drop all off at step: " + str(i - prev_terminated_step)
			prev_terminated_step = i
			PD_world, pickups, dropoffs = init_PD_world()
			current_state = '(1,5)'
			#plot(Q)		
	print_Q(Q)
...
def exp2(alpha, gamma):	
	...
	for i in range(200):		
		if dropoffs['(4,4)*'] < 8 or dropoffs['(5,1)*'] < 8:
			...
			if action == 'p':
				...
				if dropoffs[current_state] == 8:
					PD_world[current_state].pop('d')
					print "Q after dropping 8 blocks"
					print_Q(Q)
					print "============================"
	...
	for i in range(5800):
		if dropoffs['(4,4)*'] < 8 or dropoffs['(5,1)*'] < 8:
			...
			# if action == 'p':
			# 	print "Pick up at: " + str(current_state) + " at step: " + str(i)
			# if action == 'd':
			# 	print "Drop off at: " + str(current_state) + " at step: " + str(i)
			next_state = PD_world[current_state][action]
...
def init_PD_world():
	...
	return PD_world, pickups, dropoffs
